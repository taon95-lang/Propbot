...
async def scan(ctx, player=None, line=None, opponent="N/A"):
    ...
    async with ctx.typing():
        try:
            ...
            embed.add_field(name="⭐ Rating 3.0", value=data.get('Rating 3.0', 'N/A'), inline=True)
            embed.add_field(name="🎯 KAST", value=data.get('KAST', 'N/A'), inline=True)
            ...
            # Advanced Stats
            embed.add_field(name="🔫 KPR", value=data.get('KPR', 'N/A'), inline=True)
            embed.add_field(name="💀 DPR", value=data.get('DPR', 'N/A'), inline=True)
            embed.add_field(name="💥 Impact", value=data.get('Impact', 'N/A'), inline=True)
            
            embed.add_field(name="🎯 HS%", value=f"{data.get('HS %', 0)}%", inline=True)
            embed.add_field(name="💨 ADR", value=data.get('ADR', 'N/A'), inline=True)
            embed.add_field(name="🔁 Multi-kill%", value=data.get('Multi-kill %', 'N/A'), inline=True)
            ...
            if scenarios:
                ...
                scenario_text = f"Short: {short.get('expected_kills', 0)}k\nNormal: {normal.get('expected_kills', 0)}k\nLong: {long_s.get('expected_kills', 0)}k"
                embed.add_field(name="📊 Match Scenarios", value=scenario_text, inline=True)
            ...
            # Simulation Results
            embed.add_field(name="🔫 Expected", value=data['Expected kills'], inline=True)
            ...
            if map_avgs:
                map_text = "\n".join([f"{m.title()}: {stats['avg_kills']}k avg" for m, stats in list(map_avgs.items())[:3]])
                if map_text:
