...
# =========================================================
# UPDATED FETCH (No Render=True for Free Plan)
# =========================================================
def _fetch(url):
    global _SESSION
    
    if SCRAPERAPI_KEY and "search" not in url:
        # REMOVED render=true to support ScraperAPI Free/Trial plans
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"PROXY ERROR: Status {r.status_code} (Check ScraperAPI Credits/Plan)")
        except Exception as e:
            print(f"PROXY CONNECTION ERROR: {e}")

    # Fallback for search or blocked proxy
...
def _parse_match_kills(html, player_slug):
    ...
    if len(mapholders) < 2:
        print("LOG: Skipping - Match not a finished BO3.")
        return {"maps": []}

    match_stats = soup.find(id="match-stats")
    if not match_stats:
        print("CRITICAL: Stats div not found. HLTV likely blocked the proxy.")
        return {"maps": []}

    # Identify map content divs (Map 1 & 2)
    ...
    for content in map_containers[:2]:
        ...
        if player_row:
            # Look for "22 (11)" pattern for Kills (HS)
...
def get_player_info(player_name, opponent=None):
    ...
    print(f"STARTING GOLD SCAN: {display} (CS2 ONLY)")
    ...
    for mid, mslug in valid_ids:
        time.sleep(1)
        # Fetching match pages without render_js=True
